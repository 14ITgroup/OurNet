...
public partial class admin_applications : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            ApplicationBind(currentPage, pageSize);
        }
    }
    ...
    protected void RptArticles_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "delete") //删除文章
        {
            ...
            {
                applications work = db.applications.SingleOrDefault(a => a.id == id);
                db.applications.Remove(work);
                db.SaveChanges();
            }
            ApplicationBind(getPageNum(), getPageSize());
            ClientScript.RegisterStartupScript(this.GetType(), "", "<script>alert('删除成功');</script>");
        }
    }
    void ApplicationBind(int CurrentPage, int PageSize) //文章绑定
    {
        using (var db = new ITStudioEntities())
        {
    ...
    {
        ApplicationBind(1, getPageSize()); //从第一页绑定，防止单页项目数量变多，导致页码超出范围。
        TxtPageNum.Text = "1";
        Session["pagenum"] = 1;
    }
    ...
    protected void BtnPreviousPage_Click(object sender, EventArgs e)
    {
        ...
        ApplicationBind(pageNum, pageSize);
